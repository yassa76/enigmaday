    ...
    <div className="fade-in">
      ...
      {view==="iscritti" && (
        <div>
          ...
          {profiles.length===0 ? <div className="card" style={{textAlign:"center",padding:40}}><div style={{fontSize:48}}>👤</div><p style={{color:COLORS.muted,marginTop:12}}>Nessun iscritto</p></div> : (
            <div style={{background:COLORS.card,borderRadius:20,overflow:"hidden",border:`1px solid ${COLORS.cardLight}`}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead style={{background:COLORS.cardLight}}>
                  <tr>
                  ...
                  {profiles.map((u,i)=>{
                    ...
                    return (
                      <tr key={u.id} style={{background:i%2===0?"transparent":COLORS.cardLight+"44"}}>
                        ...
                        <td style={tdStyle}><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{(u.preferenze||[]).map(p=><span key={p} style={{background:CAT_COLORS[p]+"33",color:CAT_COLORS[p]||COLORS.primary,padding:"2px 8px",borderRadius:50,fontSize:11,fontWeight:700}}>{p}</span>)}</div></td>
                        ...
                        <td style={{...tdStyle,textAlign:"center"}}><span style={{fontFamily:"'Fredoka One'",fontSize:18,color:COLORS.secondary}}>{ut.length}</span></td>
