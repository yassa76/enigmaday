...
function generateShareImage({ enigma, corretto, tentativo, tempoUsato, streak, timerTotale }) {
  ...
  ctx.fillStyle = catColor; ctx.font = "bold 14px sans-serif";
  ctx.fillText(enigma.categoria, 60, 131);
  ...
  for (const word of words) {
    ...
    if (ctx.measureText(test).width > maxW && line) {
      ctx.fillText(line.trim(), 40, y); line = word + " "; y += 32;
      if (y > 270) { ctx.fillText("...", 40, y); break; }
    } else line = test;
  }
  ...
  const resultY = 320;
  if (corretto) {
    ctx.fillStyle = "#22C55E"; ctx.font = "bold 32px sans-serif";
    ctx.fillText("✅ Risolto!", 40, resultY);
  } else {
    ctx.fillStyle = "#EF4444"; ctx.font = "bold 32px sans-serif";
    ctx.fillText("❌ Non risolto", 40, resultY);
  }

  // Stats
  ctx.font = "18px sans-serif"; ctx.fillStyle = "#8888AA";
  const stats = [];
  if (corretto && tempoUsato) {
    const m = Math.floor(tempoUsato/60), s = tempoUsato%60;
    stats.push(`⏱ ${m>0?m+"m ":""}${s}s`);
  }
  if (streak > 0) stats.push(`🔥 Streak: ${streak}`);
  stats.push(DIFF_LABELS[enigma.difficolta]);
  ctx.fillText(stats.join("   ·   "), 40, resultY+36);

  // CTA
  ctx.fillStyle = "#FF6B35"; ctx.font = "bold 18px sans-serif";
  ctx.fillText("enigmaday.netlify.app", 40, 440);
  ctx.fillStyle = "#8888AA"; ctx.font = "16px sans-serif";
  ctx.fillText("Riesci a battermi? →", 340, 440);
  ...
  return (
    <div style={{ marginBottom:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
        ...
        <span style={{ fontSize:15, fontWeight:800, color, fontFamily:"'Fredoka One'" }}>
...
export default function HomePage({ enigma, yesterdayEnigma, session, profile, showToast, onLoginRequest, diffConfig, catConfig }) {
  const [answer, setAnswer] = useState("");
  const [state, setState] = useState("idle"); // idle | started | correct | wrong | timeout
  ...
  const loadSolutori = async (id, setter) => {
    const { data } = await supabase.from("enigmi_stats").select("solutori").eq("id", id).single();
  ...
  const submit = async () => {
    ...
    clearTimer();
    setTempoUsato(elapsed);

    const { data, error } = await supabase.from("tentativi").upsert(
    ...
    if (corretto) {
      setState("correct");
      loadSolutori(enigma.id, setSolutori);
      await updateStreak();
      // genera immagine
      const img = generateShareImage({ enigma, corretto:true, tempoUsato:elapsed, streak:profile?.streak||0, timerTotale });
      setShareImg(img);
    } else {
      setState("wrong");
      setShaking(true);
      setTimeout(() => setShaking(false), 500);
      const img = generateShareImage({ enigma, corretto:false, tempoUsato:elapsed, streak:profile?.streak||0, timerTotale });
      setShareImg(img);
    }
  };
  ...
  const updateStreak = async () => {
    ...
    else if (last === today) return; // già aggiornato oggi
  ...
  const shareRisultato = async () => {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare({ files:[file] })) {
        await navigator.share({ files:[file], title:"EnigmaDay", text:"Ho risolto l'enigma di oggi!" });
      } else {
        const a = document.createElement("a"); a.href = shareImg; a.download = "enigmaday.png"; a.click();
        ...
        <>
          ...
          {state === "correct" && (
            <div className="card fade-in" style={{textAlign:"center",marginBottom:20,background:`linear-gradient(135deg,${COLORS.success}22,${COLORS.card})`,border:`2px solid ${COLORS.success}`}}>
              ...
              {tempoUsato && <p style={{color:COLORS.muted,fontSize:14,marginTop:4}}>⏱ Tempo impiegato: <strong style={{color:COLORS.accent}}>{tempoUsato}s</strong></p>}
