...
function AvatarSmall({ src, emoji, nome }) {
  if (src) return (
    <img src={src} alt="avatar" style={{width:32,height:32,borderRadius:"50%",objectFit:"cover",border:`2px solid ${COLORS.primary}`}}/>
  );
  if (emoji && emoji.length <= 2) return (
    <div style={{width:32,height:32,borderRadius:"50%",background:"#16213E",border:`2px solid ${COLORS.primary}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>
      {emoji}
    </div>
  );
  return (
    <div style={{width:32,height:32,borderRadius:"50%",background:COLORS.primary,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff"}}>
...
export default function Navbar({ profile, session, onLogout, onNav, isAdmin }) {
  ...
  const avatarSrc = profile?.avatar_url || googleAvatar || null;
