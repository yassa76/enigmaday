...
const Divider = () => (
  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
    <div style={{flex:1,height:1,background:"#16213E"}}/>
    <span style={{color:"#8888AA",fontSize:13}}>oppure</span>
    <div style={{flex:1,height:1,background:"#16213E"}}/>
  </div>
);
...
export function RegisterPage({ onRegister, onLogin }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await onRegister(nome, cognome, email, pw, prefs);
    setLoading(false);
  };
      ...
      <div className="card" style={{border:`2px solid ${COLORS.secondary}33`}}>
        ...
        <form onSubmit={handleSubmit} autoComplete="on">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            <div>
              <label style={{fontWeight:700,color:COLORS.muted,fontSize:12,letterSpacing:1,display:"block",marginBottom:6}}>NOME *</label>
              <input className="input" placeholder="Mario" value={nome} onChange={e=>setNome(e.target.value)} type="text" autoComplete="given-name" required/>
            </div>
            <div>
              <label style={{fontWeight:700,color:COLORS.muted,fontSize:12,letterSpacing:1,display:"block",marginBottom:6}}>COGNOME *</label>
              <input className="input" placeholder="Rossi" value={cognome} onChange={e=>setCognome(e.target.value)} type="text" autoComplete="family-name" required/>
            </div>
          </div>
          <label style={{fontWeight:700,color:COLORS.muted,fontSize:12,letterSpacing:1}}>EMAIL *</label>
          <input className="input" style={{marginTop:6,marginBottom:16}} placeholder="tua@email.it" value={email} onChange={e=>setEmail(e.target.value)} type="email" autoComplete="email" required/>
          ...
          <label style={{fontWeight:700,color:COLORS.muted,fontSize:12,letterSpacing:1,display:"block",marginBottom:10}}>PREFERENZE ENIGMI (opzionale)</label>
