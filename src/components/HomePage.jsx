...
export default function HomePage({ enigma, session, profile, showToast, onLoginRequest }) {
  ...
  const [state, setState] = useState("idle"); // idle | correct | wrong | revealed
          ...
          {!session ? (
            ...
            <>
              ...
              {state === "wrong" && !showSolution && (
                <div className="fade-in" style={{marginTop:16,display:"flex",gap:10,flexWrap:"wrap"}}>
                  <p style={{color:COLORS.error,fontWeight:700,width:"100%"}}>❌ Non è corretto, riprova!</p>
